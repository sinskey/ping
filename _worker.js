...
// Variables
let serviceName = "ping";
let APP_DOMAIN = "sinskey.workers.dev";
...
const SUB_PAGE_URL = "https://ping.sinskey.workers.dev/sub";
...
function subPageHTML(host) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>${host} /sub</title>
<style>
...
<body>
  <h1>Subscription Generator</h1>
  <div class="hint">Semua request diproses oleh endpoint <code>/api/v1/sub</code> di domain ini.</div>

  <div class="row">
    <label>CC <input id="cc" placeholder="SG,JP,US (opsional)"/></label>
    <label>Port <input id="port" value="443,80"/></label>
    <label>VPN <input id="vpn" value="trojan,vmess,ss"/></label>
  ...
  <textarea id="out" placeholder="Hasil akan muncul di sini..."></textarea>
...
<script>
  document.getElementById('gen').onclick = async () => {
    ...
    params.set('port', document.getElementById('port').value.trim() || '443,80');
    params.set('vpn', document.getElementById('vpn').value.trim() || 'trojan,vmess,ss');
    params.set('limit', document.getElementById('limit').value.trim() || '10');
    params.set('format', document.getElementById('format').value);

    const res = await fetch('/api/v1/sub?' + params.toString());
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/sub" || url.pathname === "/sub/") {
        return new Response(subPageHTML(APP_DOMAIN), {
          status: 200,
          headers: {
            ...CORS_HEADER_OPTIONS,
